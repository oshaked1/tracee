...
func (cPathRes *ContainerPathResolver) GetHostAbsPath(mountNSAbsolutePath string, mountNS int) (
	...
	for _, pid := range pids {
		...
		return fmt.Sprintf("%s%s", procFSRoot, mountNSAbsolutePath), nil
	}
	...
	procFSFRoot, err := cPathRes.getProcessFSRoot(pid)
	if err != nil {
	...
	return fmt.Sprintf("%s%s", procFSFRoot, mountNSAbsolutePath), nil
}
