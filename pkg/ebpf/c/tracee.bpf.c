...
statfunc bool is_file_outside_ns(struct task_struct *task, struct file *file, void *syscall_pathname)
{
    ...
    // File belongs to a mount inside of the task's mount namespace
    if (file_mnt_ns == 0 || file_mnt_ns == task_mnt_ns)
        return false;
    
    // Child reaper is PID 1 of the PID namespace.
    struct pid_namespace *pid_ns = get_task_pid_ns(task);
    struct task_struct *pid_ns_leader = BPF_CORE_READ(pid_ns, child_reaper);

    // Check if the namespace leader's mount NS is the one we are accessing.
    // This could happen if a container is started in the host PID namespace,
    // in which case file accesses via procfs links could be to a host mount.
    // If this is the case and the file is being accessed via a procfs link,
    // we consider this access to be within the task's namespaces.
    char unresolved_path[7] = {0}; // enough to hold "/proc/"
    bpf_probe_read_str(unresolved_path, 7, syscall_pathname);
    if (file_mnt_ns == get_task_mnt_ns_id(pid_ns_leader) && strncmp(unresolved_path, "/proc/", 6) == 0)
        return false;
    
    return true;
}

statfunc bool is_file_outside_mount(struct file *file)
{
    struct path path = get_path_from_file(file);
    struct mount *mount = real_mount(path.mnt);

    // Special mount in namespace 0, ignore
    if (BPF_CORE_READ(mount, mnt_ns, ns.inum) == 0)
        return false;

    // Traverse the dentry ancestors until we find the mount root
...
#pragma unroll
    ...
    // We didn't find the mount root - this file was not supposed to be accessible via this mount
    if (dentry != root)
        return true;
...
{
    program_data_t p = {};
    if (!init_program_data(&p, ctx, OPEN_FILE_OUTSIDE_NS))
        return 0;
    ...
    if (evaluate_scope_filters(&p) && is_file_outside_ns(p.event->task, file, syscall_pathname)) {
        struct path path = get_path_from_file(file);
        struct mount *mount = real_mount(path.mnt);
        u32 file_mnt_ns = BPF_CORE_READ(mount, mnt_ns, ns.inum);
    ...
    if (reset_event(p.event, OPEN_FILE_OUTSIDE_MOUNT) && evaluate_scope_filters(&p) &&
        is_file_outside_mount(file)) {
        save_str_to_buf(&p.event->args_buf, file_path, 0);
        save_to_submit_buf(&p.event->args_buf, &flags, sizeof(flags), 1);
        save_str_to_buf(&p.event->args_buf, syscall_pathname, 2);
        ...
        // Get mount root path (mount_src arg), use a new string buffer so we don't overwrite file_path
        ...
        // Get mountpoint path (mount_dst arg), reuse the previous string buffer
