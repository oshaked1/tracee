...
func populateMapsStackTrace(eventFilters map[string]filters.Filter[*filters.StringFilter], bpfModule *bpf.Module) error {
	...
	if err := updateStackTraceTailCalls(bpfModule, "su_tail_kp", "stack_unwind_kp", selectedEvents); err != nil {
		return err
	}
	if err := updateStackTraceTailCalls(bpfModule, "su_tail_tp", "stack_unwind_tp", selectedEvents); err != nil {
		return err
	}
